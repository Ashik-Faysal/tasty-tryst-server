...
async function run() {
  try {
    ...
    const reviewCollection = client.db("tasty-tryst").collection("reviews");
    const cartCollection = client.db("tasty-tryst").collection("carts");
    const paymentCollection = client.db("tasty-tryst").collection("payments");
...
app.listen(port, () => {
  console.log(`Tasty Tryst server running on port ${port}`);
});
